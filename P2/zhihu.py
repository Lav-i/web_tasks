    ...
    while offset <= 100:#测试爬取偏移量最大为100的所有回答
        ...
        for img in imgs:
            try:
                ...
                urlStr = img['src']
                urlStr = urlStr[2:-2]
                res = urllib.request.urlopen(urlStr , timeout = 5)#设置5s超时
            ...
            with open(fileName,'wb') as f:
                f.write(res.read())
                #print(threading.current_thread().name)#线程几正在工作
...
#创建两个线程共同爬取数据
t1 = threading.Thread(target=spider)
...
print('运行结束')
print('耗时：',int(finishTime-startTime),'s')